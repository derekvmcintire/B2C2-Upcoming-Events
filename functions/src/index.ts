...
export const submitEvent = functions.https.onCall(async (data: unknown) => {
  try {
  ...
  async (data: unknown) => {
    try {
      ...
      const response = await fetch(
        `https://www.crossresults.com/api/b2c2lookup.php?eventID=${request.eventId}`
      );
      ...
      throw new functions.https.HttpsError(
        "internal",
        "Error fetching registered riders"
      );
    }
  }
);
