...
const allowedOrigins = [
  "https://b2-c2-upcoming-events.vercel.app",
  "http://localhost:5173",
];
...
function prepareRequest(
  ...
  const requestOptions: RequestInit = {
    ...
    body:
      req.method !== "GET" && req.body ? JSON.stringify(req.body) : undefined, // Include body if not a GET request
  };
  ...
  if (requestOptions && isSecuredApi(apiUrl)) {
    (requestOptions.headers as Record<string, string>)["x-api-key"] =
      process.env.VITE_API_SECRET_KEY!; // Securely add your API key
  }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (typeof apiUrl !== "string") {
    return res
      .status(400)
      .json({ error: 'Missing or invalid "apiUrl" query parameter' });
  }
  ...
  const requestOptions = prepareRequest(
    req,
    res,
    `${apiUrl}${queryParams}`,
    headers as string,
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: `Request failed: ${(error as Error).message}` });
  }
}
